				...
				for (int cas = 0; cas < 3; ++cas)
				{
					...
					for (int met = 0; met < 2; met++)
					{
						...
						if (typ == 0)
						{
							...
							CI_train = FindCI(seedsayisi, traincost, 1);

							//Get the best performing train results
							for (int job = 0; job < n; ++job)
								for (int t = 0; t < tt; ++t)
									z1best[job][t] = z1_all[index][job][t];
						}
						//model2 Test
						...
						for (int k = 0; k < K; ++k)
						{
							...
							if (typ == 0)
								testcost[k] = SetupModelTest(z1best, n, testscenarios, prevdec, cordec, opernum, gammanum, sla_v, cornum, prevnum, opercost, Cadd, pensla, Cc, Cp, Yp, Yc, L, SLA);
							else
								testcost[k] = SetupModelTest(z2, n, testscenarios, prevdec, cordec, opernum, gammanum, sla_v, cornum, prevnum, opercost, Cadd, pensla, Cc, Cp, Yp, Yc, L, SLA);
						...
						if (typ == 0)
						{
							resultfile << "\n #prev\t" << "#cor \t" << "#oper \t" << "#sla_v \t" << "track_v\t" << "testcost\t" << "traincost\t" << "traintime\t" << "Testime\t"
								<< "CI for LB \t" << "CI for UB \t" << "Gap% \t" << "#prevdec \t " << "#cordec \t " << "seed \n";

							resultfile << prev << "\t" << cor << "\t" << oper << "\t" << slaviol << "\t" << trackviol << "\t" <<
								avgtestcost << "\t" << avgtrain << "\t" << tottraintime << "\t" << totaltestime << "\t"
								<< "[" << CI_train[0] << ";" << CI_train[1] << "]\t"
								<< "[" << CI_test[0] << ";" << CI_test[1] << "]\t" << gapercent << "\t" << prevbeg << "\t" << corbeg <<
								"\t" << seed << "\n\n";
						}
						...
						else
						{
							...
							resultfile << prev << "\t" << cor << "\t" << oper << "\t" << slaviol << "\t" << trackviol << "\t" <<
								avgtestcost << "\t" << model2cost << "\t" << tottraintime << "\t" << totaltestime << "\t"
